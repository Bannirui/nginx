...
struct ngx_pool_s {
    ...
    ngx_pool_data_t       d;
    /**
     * 向内存池申请空间 每次申请可以分配的上限
     * 取决于内核页大小和内存池实际大小
     * <ul>
     *   <li>内存页大小是4KB</li>
     *   <li>如果内存池很大 超过4KB 那么每次可以分配的大小上限就是内存页大小 也是4KB</li>
     *   <li>如果内存池很小 小于4KB 那么每次可以分配的大小上限就是内存池数据区空间</li>
     * </ul>
     * 内存池实例化的时候就决定好这个属性了
     * 以此界定要申请的空间属性小内存还是大内存
     * <ul>
     *   <li>要申请的内存空间<=max 小空间就放在内存池内管理</li>
     *   <li>要申请的内存空间>max 在空间额外开辟在内存池外</li>
     * </ul>
     */
    size_t                max;
    /**
     ...
     * </ul>
     * 这个current是作用就是指向每次分配内存从哪个内存块开始遍历 current滑动的机制靠内存块分配失败计数
     */
    ngx_pool_t           *current;
    ngx_chain_t          *chain;
    /*
